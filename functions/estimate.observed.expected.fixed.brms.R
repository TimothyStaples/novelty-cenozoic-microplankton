estimate.observed.expected.fixed.brms <- function(prob.model.list,
                                            novel.list){
  ...
  base.prob.models <- lapply(prob.model.list$fixed.prob.models,
  ...
  exp.trans.list <- lapply(1:16, function(n){
    ...
    exp.post <- sapply(1:ncol(exp.post1), function(n1){
    
      exp.post <- plogis(exp.post1[,n1]) * plogis(exp.post2[,n1])
      
      exp.post <- log(exp.post / (1 - exp.post))
    })
  colnames(exp.post) <- colnames(exp.post1)
  ...
  exp.trans <- exp.trans.list[[n]]
    ...
    trans.m <- brm(success | trials(trials) ~ -1 + taxa,
  ...
  obs.post <- posterior_samples(trans.preds[[n]]$model, pars = "b")[,1:4]
  exp.post <- posterior_samples(exp.trans.list[[n]])
 
  total.df <- do.call("rbind", lapply(1:ncol(obs.post), function(n1){
  
  diff.post <- plogis(obs.post[,n1]) / plogis(exp.post[,n1])
  
  diff.df <- data.frame(obs.mean = plogis(mean(obs.post[,n1])),
             obs.upper = plogis(quantile(obs.post[,n1], 0.975)),
             obs.lower = plogis(quantile(obs.post[,n1], 0.025)),
             
             exp.mean = plogis(mean(exp.post[,n1])),
             exp.upper = plogis(quantile(exp.post[,n1], 0.975)),
             exp.lower = plogis(quantile(exp.post[,n1], 0.025)),
  ...
  diff.df <- cbind(trans.list[n,], 
                   taxa = gsub("b_taxa", "", colnames(obs.post)[n1]),
                   diff.df)
  return(diff.df)
  }))
