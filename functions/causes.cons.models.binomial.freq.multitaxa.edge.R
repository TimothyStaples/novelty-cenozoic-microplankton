causes.cons.models.binom.edge <- function(success.var,
                                          trial.var,
                                          edge.var = NA,
                                          turnover.df){
  ...
  print("Running model...")
  if(is.na(edge.var)){
  cc.model <-  glmer(cbind(success, failure) ~ (cat.bef + cat.aft) + lag.scaled + 
                     (1|taxa/site),
    ...
    cc.model <-  glmer(cbind(success, failure) ~ (cat.bef + cat.aft) + 
                       lag.scaled + edge.scaled + (1|taxa/site),
                       family=binomial, data=turnover.df,
                       control=glmerControl(optimizer="bobyqa"))
